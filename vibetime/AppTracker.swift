...
class AppTracker: ObservableObject {
    ...
    private func handleAppActivated(bundleID: String, appName: String) {
        ...
        // Count context switch if coming from a different tracked app
        if let previous = currentActiveBundle, previous != bundleID {
            totalContextSwitches += 1
            // Reset focus streak on the previous app
